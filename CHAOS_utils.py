...
def cal_dice(output, target):
    ...
    dice1(liver):label0
    dice2(l_kidney):label1
    dice3(r_kidney): label2
    dice4(splenic): label3

    '''
    output = torch.argmax(output, dim=1)

    print(f'LOSS ----- outputs and targets dtype:{output.dtype, target.dtype}')

    dice1 = Dice((output == 1).float(), (target == 1).float())
    dice2 = Dice((output == 2).float(), (target == 2).float())
    dice3 = Dice((output == 3).float(), (target == 3).float())
    dice4 = Dice((output == 4).float(), (target == 4).float())
...
class Loss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        target1 = F.one_hot(target.to(torch.int64), self.n_classes)
        ...
        # print(f'input and target shape:{input.shape,target.shape}')
        # print(f'input and target dtype:{input.dtype, target.dtype}')
