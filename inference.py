...
def test_all_case(net, image_list, num_classes=2, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True,
    ...
    for ith, image_path in enumerate(image_list):
        ...
        # print(f'save path:{test_save_path + "%02d_pred.nii.gz" % (ith)}')



        if save_result:
            # nib.Nifti1Image(prediction.astype(np.float32), np.eye(4)).to_filename(test_save_path + "%02d_pred.nii.gz" % (ith))
            nib.save(nib.Nifti1Image(prediction.astype(np.float32), np.eye(4)),
                     test_save_path + "%02d_pred.nii.gz" % (ith))
            # print(f'保存成功')
            # modalities = ['flair', 't1ce', 't1', 't2']
            # image只保留一个模态
            nib.save(nib.Nifti1Image(image[0].astype(np.float32), np.eye(4)),
...
if __name__ == '__main__':
    ...
    test_save_path = '/mnt/sda3/yigedabuliu/lkq/brats-unet/predictions/unet/'
    save_mode_path = '/mnt/sda3/yigedabuliu/lkq/brats-unet/checkpoint/UNet/checkpoint60.pth'
    net = UNet(in_channels=4, num_classes=4).cuda()
    net.load_state_dict(torch.load(save_mode_path)['model'])
    print("init weight from {}".format(save_mode_path))
    net.eval()
    with open('/mnt/sda3/yigedabuliu/lkq/data/Task01_BrainTumour/test.txt', 'r') as f:
