...
from datasets.Brats_datasets import *
from networks.BratsUnet import UNet
from loss.brats_loss import Loss, cal_dice, cosine_scheduler
import os
...
def main(args):
    ...
    # data info
    patch_size = (160, 160, 128)  # 数据裁切后的图像大小
    # 构建自定数据类
    train_dataset = BraTS_SingleModal(args.data_path, args.train_txt, transform=transforms.Compose([
        RandomRotFlip(),
        RandomCrop(patch_size),
        GaussianNoise(p=0.1),
        ToTensor()
    ]), modal=args.modal)
    val_dataset = BraTS_SingleModal(args.data_path, args.valid_txt, transform=transforms.Compose([
        CenterCrop(patch_size),
        ToTensor()
    ]), modal=args.modal)
    test_dataset = BraTS_SingleModal(args.data_path, args.test_txt, transform=transforms.Compose([
        CenterCrop(patch_size),
        ToTensor()
    ]), modal=args.modal)
    # 数据迭代器
    ...
    # 1-坏疽(NT,necrotic tumor core),2-浮肿区域(ED,peritumoral edema),4-增强肿瘤区域(ET,enhancing tumor) 0-背景
    # 评价指标：ET(label4，高度恶性活跃区域),TC(label1+label4，肿瘤的实质部分),WT(label1+label2+label4，肿瘤的总体大小和分布情况)
    model = UNet(in_channels=1, num_classes=4).to(device)  # 4个类别包含了背景
    ...
    print("Valid -- loss: {:.3f} ET: {:.3f} TC: {:.3f} WT: {:.3f}".format(metrics2['loss'], metrics2['dice1'],
                                                                          metrics2['dice2'], metrics2['dice3']))
    print("Test  -- loss: {:.3f} ET: {:.3f} TC: {:.3f} WT: {:.3f}".format(metrics3['loss'], metrics3['dice1'],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--modal', type=str, default='flair', help='flair,t1ce,t1,t2')

    parser.add_argument('--min_lr', type=float, default=0.002)
    parser.add_argument('--data_path', type=str,
                        default='/mnt/sda3/yigedabuliu/lkq/data/Task01_BrainTumour/h5_datasets')
    parser.add_argument('--train_txt', type=str, default='/mnt/sda3/yigedabuliu/lkq/data/Task01_BrainTumour/train.txt')
    parser.add_argument('--valid_txt', type=str, default='/mnt/sda3/yigedabuliu/lkq/data/Task01_BrainTumour/valid.txt')
    parser.add_argument('--test_txt', type=str, default='/mnt/sda3/yigedabuliu/lkq/data/Task01_BrainTumour/test.txt')
    parser.add_argument('--train_log', type=str, default='results/UNet.txt')

    parser.add_argument('--weights', type=str, default='')
    parser.add_argument('--save_path', type=str, default='checkpoint/UNet_Brats_single_modal')
    ...
    args.save_path = 'checkpoint/' + 'Brats/' + args.modal
